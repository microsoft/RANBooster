...
int main(int argc, char **argv) {
    ...
    // Find the MAC address map
    map = bpf_object__find_map_by_name(obj, "du_mac_address_ru");
    if (!map) {
        fprintf(stderr, "Failed to find MAC address map\n");
        bpf_object__close(obj);
        return 1;
    ...
    parse_mac_address(du_mac_ru, du_mac_addr);
    ...
    // Find the MAC address map
    map = bpf_object__find_map_by_name(obj, "du_mac_address_local");
    if (!map) {
        fprintf(stderr, "Failed to find MAC address map\n");
        bpf_object__close(obj);
        return 1;
    ...
    parse_mac_address(du_mac_middlebox, ranbooster_du_mac_addr);
